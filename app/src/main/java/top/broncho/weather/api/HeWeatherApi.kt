package top.broncho.weather.api

import retrofit2.http.GET
import retrofit2.http.Headers
import retrofit2.http.Query
import top.broncho.weather.BuildConfig
import top.broncho.weather.model.BaseResponse
import top.broncho.weather.model.CityBean
import top.broncho.weather.util.DOMAIN_NAME

const val BASE_URL_WEATHER = "https://free-api.heweather.net/s6"
const val BASE_URL_SEARCH = "https://search.heweather.net"
...
interface HeWeatherApi {
     ...
     * @return 搜索到的城市结果
     */
    @Headers("$DOMAIN_NAME: $BASE_URL_SEARCH")
    @GET("/find?")
    suspend fun searchCity(
        @Query("key") key: String = BuildConfig.HEWEATHER_KEY,
        @Query("location") location: String
    ): BaseResponse<CityBean>
     ...
     * @return
     */
    @Headers("$DOMAIN_NAME: $BASE_URL_WEATHER")
    @GET("/weather?")
    fun getWeather(
        @Query("key") key: String = BuildConfig.HEWEATHER_KEY,
        @Query("location") location: String
